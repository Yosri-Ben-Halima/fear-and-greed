...
def parse_perps(input_data: dict) -> dict:
    ...
    output_data = {
        ...
        "open_interest": points.get("0", {}).get("open_interest"),  # Extracting open interest
        "volume": points.get("0", {}).get("volume"),  # Extracting volume
        "funding": points.get("0", {}).get("funding"),  # Extracting basis
        "liquidations_long": points.get("0", {}).get("liquidations_long"), # Extracting liquidations long side
        "liquidations_short": points.get("0", {}).get("liquidations_short"), # Extracting liquidations short side
    }
